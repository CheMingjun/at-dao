            ...
            _rows.forEach(function (row) {
                ...
                for (let k in _mp) {
                    ...
                    if (typeof tm == 'function') {
                        ...
                        throw new Error("The mapping for field[" + k + "] must be a function or a string or undefined,the row's json from table is:" + JSON.stringify(row));
                    }
                }
                rtn.push(pj);
            })
        ...
        if (typeof dsCfg === 'object') {
            if (!dsCfg.db || typeof(dsCfg.db) === 'string' && dsCfg.db.toUpperCase(dsCfg.db) === 'MYSQL') {
                ...
                try {
                    ...
                    logger.info('[at-dao] Database[' + _name + '][host:' + dsCfg.host + '] start finished.');
                } catch (err) {
                    throw err;
                }
            }
...
module.exports = {
    initPool: function (_cfg) {
        ...
        if (typeof inst === 'undefined') {
            dsReg[DefDSName] = new ds(DefDSName, _cfg);
        }
    }, shutdown: function*() {
        if (dsReg) {
